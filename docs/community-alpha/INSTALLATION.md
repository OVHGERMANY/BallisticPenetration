...
2. Verify the downloaded ZIP against its accompanying `.zip.sha256.txt` file.
3. Back up any existing `BepInEx\plugins\BallisticPenetration` folder and `BepInEx\config\com.janky.ballisticpenetration.cfg`.
4. Extract the package into the SPT root. The package places the DLL at:

   `BepInEx\plugins\BallisticPenetration\BallisticPenetration.dll`

5. Start SPT normally.
6. Confirm the BepInEx log contains `Janky-BallisticPenetration 1.3.0` and no load failure.

The normal terminal-ballistics path is enabled by default. The experimental physical-projectile path remains disabled. To enable it, close the game, edit:
...
5. Start SPT and confirm the BepInEx log reports version `1.3.0` without a load failure.
